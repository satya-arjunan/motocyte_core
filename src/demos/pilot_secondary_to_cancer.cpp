...
int main() {
  ...
  VolumeSpecies D("D", model, 3650, 0);
  D.get_populator().set_cuboid(0.2, 1.0, 1.0);
  D.get_populator().set_origin(0.40, 0, 0);
  VolumeSpecies A("A", model, 1550, 0);
  A.get_populator().set_cuboid(0.1, 1.0, 1.0);
  A.get_populator().set_origin(0.25, 0, 0);

  VolumeSpecies B("B", model, 800, 1e-12);
  VolumeSpecies C("C", model, 800, 1e-12);
