...
int main() {
  ...
  VolumeSpecies B("B", model, 50, 1e-12);
  VolumeSpecies C("C", model, 10, 1e-12);
  ...
  MesoSpecies Bm2("Bm2", model, 0, 1e-12);

  Reaction exocytosis1(Ap, 5, Am1); //Ap -> Am1
  exocytosis1.set_interval(1e-5);
  Reaction exocytosis2(Bp, 5, Bm2); //Ap -> Am1
  exocytosis2.set_interval(1e-7);
  ...
  Reaction degradation2(Bm, 20000); //Bm-> 0
  degradation2.set_interval(1e-5);
  Reaction degradation3(Bm2, 5000000); //Bm-> 0
  degradation3.set_interval(1e-9);

  model.get_writer().set_interval(1e-7);
