...
XMLModelReader::XMLModelReader(std::string file) {
...
void XMLModelReader::parse_file() {
}

Vector<double>& XMLModelReader::get_dimensions() {
  std::cout << "dimensions:" << dimensions_.x << " " << dimensions_.y << " " 
    << dimensions_.z << std::endl;
  return dimensions_;
}

unsigned XMLModelReader::get_size() {
  return size_;
}

double XMLModelReader::get_interval() const {
  return interval_;
} 

double XMLModelReader::get_end_time() const {
  return interval_*steps_;
} 
